...
author: 0xCodez
aliases:
  - 阅读列表/2026年05月/claude-agent-team-guide
---
...
Netflix 平台团队已将此模式用于生产：多 agent 编排处理数百个并发构建的日志。架构约束：仅一层委派（子 agent 不能再带团队）。

五个常见错误：不需要团队时硬组、协调者自己动手干活、工具范围不严格限定、对抗 depth-1 限制（而非接受它）、无 trace 盲跑。
