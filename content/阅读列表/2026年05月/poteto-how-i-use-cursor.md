...
一个 Cursor Cloud Agent，全自动处理 bug：
1. 理解 bug 报告（图片/视频都行）
2. 探索代码库，跟报告者聊天澄清
3. 从代码、git 历史、Slack、Notion 聚合上下文创建 ticket
4. 通过 CDP 计算机操作复现 bug
5. 修复、抓取 CPU trace/heap snapshot
6. 生成 worker 做验证，录修复前后视频
7. 自动开 PR

### 核心哲学

> "Agent 的瓶颈是 **验证**。先建立信任，再规模化。并行化你还不信任的 agent 是巨大的 token 浪费，还会引入更多 slop。"
